...
function toggleOptionVisibility(showOptions) {
    if (showOptions) {
        setDisplay('options', true); 
        setDisplay('showOptions', false); 
        setDisplay('hideOptions', true);
        optionsVisible = true;
    } else {
        setDisplay('options', false); 
        setDisplay('showOptions', true); 
        setDisplay('hideOptions', false);
 ...
 */
function convert() {
    var removeWhitespace = document.getElementById('whitespaceCheckbox').checked;
    var extractSuperbuy = document.getElementById('superbuyCheckbox').checked;
    // Convert the links in the text area and place in arrays
    var links = convertURLs(document.getElementById('content').value.split('\n'), removeWhitespace, extractSuperbuy);
    ...
    if (validLinks.length + invalidLinks.length > 0) {
        setDisplay('options', false);
        setDisplay('optionsVisibility', false);
    }
...
function reset() {
    ...
    setDisplay('optionsVisibility', true);
    setHTML('message', '');
