 ...
 */
function convertURLs(urls, removeWhitespace, extractSuperbuy) {
    console.log('Remove whitespace: ' + removeWhitespace + ', extract Superbuy links: ' + extractSuperbuy);
    ...
    urls.forEach(function (url) {
        ...
        if (extractSuperbuy && contains(url, 'superbuy.com')) {
            url_obj = new URL(url);
            url_params = url_obj.searchParams

            if (url_params.has('url')) {
                url = url_params.get('url');
                url = decodeURIComponent(url);
                console.log('URL extracted from Superbuy link: ' + url);
            }
	    }
        if (isTaobaoURL(url))
